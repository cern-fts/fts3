...
static void doServer()
{
    ...
    if (isDaemon && !logPath.empty()) {
        logPath += "/fts3optimizer.log";

        if (theLogger().redirect(logPath, logPath) != 0) {
            std::ostringstream msg;
            msg << "fts3 Optimizer daemon failed to open logfile=" << logPath << " error=" << strerror(errno);
    ...
    OptimizerServer::instance().wait();
    FTS3_COMMON_LOGGER_NEWLOG(INFO)<< "Daemon halt" << commit;
}

/// Prepare, fork and run the Optimizer daemon
static void spawnServer(int argc, char** argv)
{
    ...
    if (isDaemon) {
        int d = daemon(0, 0);
        std::cerr << "Created the fts_optimizer daemon!" << std::endl;

        if (d < 0) {
        	std::cerr << "Can't fork the daemon " << d << std::endl;
            throw SystemError("Can't fork the optimizer daemon!");
        }
    }

    createPidFile(pidDir, "fts-optimizer.pid");
...
int main(int argc, char** argv)
{
    ...
    int countRunning = countProcessesWithName("fts_optimizer");

    if (countRunning < 0) {
        std::cerr << "Could not check if the Optimizer daemon is already running!" << std::endl;
        return EXIT_FAILURE;
    }

    if (countRunning > 1) {
        std::cerr << "Only one instance of the Optimizer daemon can run at one time!" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        std::cerr << "Failed to spawn the Optimizer daemon! "
                  << "(exception=" << ex.what() << ")" << std::endl;
        return EXIT_FAILURE;
    } catch (...) {
        std::cerr << "Failed to spawn the Optimizer daemon! (unknown exception)" << std::endl;
        return EXIT_FAILURE;
    }
