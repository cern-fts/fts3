   ...
   */
     ...
     */
      ...
      function line2axisChart()
      {
        ...
        var yLeft = d3.scale.linear().range([
            height,
            10
          ]);
        var yRight = d3.scale.linear().range([
            height,
            10
          ]);
        var xAxis = d3.svg.axis().scale(x).orient('bottom');
        filterXAxis(xAxis, x);
        var yAxisLeft = d3.svg.axis().scale(yLeft).orient('left').ticks(5).tickFormat(d3.format('s'));
        var yAxisRight = d3.svg.axis().scale(yRight).orient('right').ticks(5).tickFormat(d3.format('s'));
        
        var yDataLeft  = [];
        var yDataRight = [];
        points.forEach(function (d) {
            yDataLeft.push(d.y[0]);
            yDataRight.push(d.y[1]);
        });
        
        var lineLeft = d3.svg.line()
            .x(function(d, i) {
                return Math.round(x(d.x)) + x.rangeBand() / 2;
            })
            .y(function(d) {
                return yLeft(d.y[0]);
            });
        ...
        var svg = d3.select(chartContainer[0]).append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
        var paddingLeft = 0.1 * d3.max(yDataLeft);
        var paddingRight = 0.1 * d3.max(yDataRight);
        yLeft.domain([
          0,
          d3.max(yDataLeft) + paddingLeft
        ]);
        yRight.domain([
          0,
          d3.max(yDataRight) + paddingRight
        ]);
        svg.append('g').attr('class', 'x axis').attr('transform', 'translate(0,' + height + ')').call(xAxis);
        svg.append('g').attr('class', 'y axis').call(yAxisLeft).attr('style', 'fill: ' + config.colors[0]);
        svg.append('g').attr('class', 'y axis').attr('transform', 'translate(' + width + ', 0)').attr('style', 'fill: ' + config.colors[1]).call(yAxisRight);
        
        svg.append('path').attr('d', lineLeft(points)).attr('class', 'line-left').attr('stroke-width', '2').attr('fill', 'none').attr('style', 'stroke: ' + config.colors[0]);
        svg.append('path').attr('d', lineRight(points)).attr('class', 'line-right').attr('stroke-width', '2').attr('fill', 'none').attr('style', 'stroke: ' + config.colors[1]);
        
        svg.append('text').attr('class', 'y label')
            .attr('text-anchor', 'end')
            .attr('transform', 'translate(-30, ' + height / 2 + ') rotate(-90)')
            .attr('style', 'fill: ' + config.colors[0])
            .text(config.labels[0]);
            
        svg.append('text').attr('class', 'y label')
            .attr('text-anchor', 'middle')
            .attr('transform', 'translate(' + (width + 30) + ', ' + height / 2 + ') rotate(90)')
            .attr('style', 'fill: ' + config.colors[1])
            .text(config.labels[1]);
      } 
