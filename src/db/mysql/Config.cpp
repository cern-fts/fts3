...
int MySqlAPI::getStreamsOptimization(const std::string &sourceSe, const std::string &destSe)
{
    ...
    try
    {
        ...
        "   SELECT nostreams FROM t_link_config WHERE source_se = :source AND dest_se = '*' UNION "
        "   SELECT nostreams FROM t_link_config WHERE source_se = '*' AND dest_se = :dest UNION "
        "   SELECT nostreams FROM t_link_config WHERE source_se = '*' AND dest_se = '*' UNION "
