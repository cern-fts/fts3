        ...
        for (auto i = delOpsStatus.begin(); i < delOpsStatus.end(); ++i)
        {
            ...
            else if (totalNumOfFilesInJob == numberOfFilesFailed) // all failed / job failed
            {
                sql <<
                    ...
                    "    job_state = 'FAILED',"
                    "    job_finished = " << utc_timestamp << ","
                    "    reason='Job failed, check files for more details' "
                    "WHERE job_id = :jobId",
                    soci::use(i->jobId);
            }
            ...
            else if(totalNumOfFilesInJob == totalInTerminal && numberOfFilesCanceled >= 1) //CANCELED
            {
                sql <<
                    ...
                    "    job_state = 'CANCELED',"
                    "    job_finished = " << utc_timestamp << ","
                    "    reason='Job canceled, check files for more details' "
                    "WHERE job_id = :jobId",
                    soci::use(i->jobId);
            }
            else
...
std::list<TransferFile> MySqlAPI::postgresGetScheduledFileTransfers(const int maxFiles)
{
    ...
    try
    {
        const soci::rowset<TransferFile> rs = (sql.prepare <<
            ...
            "    UPDATE t_file\n"
            "    SET file_state = 'SELECTED'\n"
            "    WHERE file_id IN (\n"
            "        SELECT f.file_id\n"
            "        FROM t_file f\n"
            "        WHERE f.file_state = 'SCHEDULED'\n"
            "        ORDER BY f.file_id\n"
            ...
            "    u.source_se,\n"
            "    u.dest_se,\n"
            "    u.selection_strategy,\n"
            "    j.internal_job_params,\n"
            "    j.job_type\n"
            "FROM updated_files u\n"
            "INNER JOIN t_job j ON u.job_id = j.job_id",
