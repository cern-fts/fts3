                              ...
                              const JobParameterHandler & params)
{
    ...
    try
    {
        ...
        for(itr = mapa.begin(); itr != mapa.end(); ++itr)
        {
            ...
            long long int linkExists = 0;
            sql << "select count(*) from t_optimize_active where source_se=:source_se and dest_se=:dest_se LIMIT 1",
                soci::use(source_se),
                soci::use(dest_se),
                soci::into(linkExists);
            if(linkExists == 0) //for some reason does not exist, add it
...
bool MySqlAPI::markAsStalled(std::vector<struct message_updater> &messages, bool diskFull)
{
    soci::session sql(*connectionPool);
...
void MySqlAPI::updateHeartBeatInternal(soci::session& sql, unsigned* index, unsigned* count, unsigned* start, unsigned* end, std::string service_name)
{
    try
    {
        auto heartBeatGraceInterval = theServerConfig().get<int>("HeartBeatGraceInterval");
