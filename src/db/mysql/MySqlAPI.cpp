...
{
    return throughput != 0.0? my_round((throughput / 1024), 1): 0.0;
}
    ...
    try
        {
		    ...
		    gmtime_r(&now, &tTime);
                    soci::rowset<TransferFiles> rs = (
                                                         ...
                                                         " FROM t_file f ignore index(file_source_dest) ignore index(t_file_select) ignore index(file_vo_name_state) INNER JOIN t_job j ON (f.job_id = j.job_id) WHERE  "
                                                         "    f.vo_name = j.vo_name AND f.file_state = 'SUBMITTED' AND  "
