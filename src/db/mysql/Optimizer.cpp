...
std::list<Pair> MySqlAPI::getActivePairs()
{
    try{
...
OptimizerMode MySqlAPI::getOptimizerMode(const std::string &source, const std::string &dest)
{
    try {
        soci::session sql(*connectionPool);
...
void MySqlAPI::getPairLimits(const Pair &pair, Range &range, StorageLimits &limits)
{
    try {
        soci::session sql(*connectionPool);
...
int MySqlAPI::getOptimizerValue(const Pair &pair)
{
    try {
        soci::session sql(*connectionPool);
...
void MySqlAPI::getThroughputInfo(const Pair &pair, const boost::posix_time::time_duration &interval,
...
time_t MySqlAPI::getAverageDuration(const Pair &pair, const boost::posix_time::time_duration &interval)
{
    try {
        soci::session sql(*connectionPool);
...
double MySqlAPI::getSuccessRateForPair(const Pair &pair, const boost::posix_time::time_duration &interval,
                                       int *retryCount)
{
    try {
        soci::session sql(*connectionPool);
...
int MySqlAPI::getCountInState(const Pair &pair, const std::string &state)
{
    try {
        soci::session sql(*connectionPool);
...
double MySqlAPI::getThroughputAsSource(const std::string &se) {
    try {
        soci::session sql(*connectionPool);
...
double MySqlAPI::getThroughputAsDestination(const std::string &se)
{
    try {
        soci::session sql(*connectionPool);
