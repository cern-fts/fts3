...
CREATE OR REPLACE PROCEDURE file_transfer_active(
    _active_file_id bigint,
    _reason varchar,
    _transfer_host varchar,
    _pid integer,
    _filesize bigint,
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE 'file_transfer_active failed: No file: file_id=%',
            _active_file_id;
    END IF;

    IF _file_row_file_state != 'READY' THEN
        RAISE 'file_transfer_active failed: Initial file state is not READY: file_id=% file_state=%',
            _active_file_id, _file_row_file_state;
    END IF;

    SELECT change_file_state_and_queues(
        _file_id => _active_file_id,
    ...
    IF NOT FOUND THEN
        RAISE 'file_transfer_active failed: Failed to update file: file_id=%',
            _active_file_id;
    END IF;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE PROCEDURE file_transfer_failed(
    _failed_file_id bigint,
    _reason varchar,
    _transfer_host varchar,
    _pid integer,
    _filesize bigint,
...
BEGIN
    ...
    FROM
        t_file
    WHERE
        file_id = _failed_file_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE 'file_transfer_failed failed: No file: file_id=%',
            _failed_file_id;
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE 'file_transfer_failed failed: Failed to update file: file_id=%',
            _failed_file_id;
    END IF;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE PROCEDURE file_transfer_canceled(
    _canceled_file_id bigint,
    _reason varchar,
    _transfer_host varchar,
    _pid integer,
    _filesize bigint,
...
BEGIN
    ...
    FROM
        t_file
    WHERE
        file_id = _canceled_file_id
    FOR UPDATE;

    IF NOT FOUND THEN
        RAISE 'file_transfer_canceled failed: No file: file_id=%',
            _canceled_file_id;
    END IF;

    IF _file_row_file_state != 'CANCELED' THEN
        SELECT change_file_state_and_queues(
            _file_id => _canceled_file_id,
            _curr_file_state => _file_row_file_state,
    ...
    IF NOT FOUND THEN
        RAISE 'file_transfer_canceled failed: Failed to update file: file_id=%',
            _canceled_file_id;
    END IF;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE PROCEDURE file_transfer_staging_finished(
    ...
    _throughput real,
    _current_failures integer,
    _staging_finished timestamp without time zone,
    _file_metadata varchar
)
AS $$
DECLARE
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE 'file_transfer_staging_finished failed: Failed to update file: file_id=%',
            _staging_finsihed_file_id;
    END IF;
END;
$$ LANGUAGE plpgsql;
