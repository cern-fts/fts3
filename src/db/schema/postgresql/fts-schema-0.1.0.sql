...
CREATE TABLE t_token (
    ...
    exchange_retry_timestamp   TIMESTAMP         NULL,
    exchange_retry_delay_m     INTEGER           NULL DEFAULT 0,
    exchange_attempts          INTEGER           NULL DEFAULT 0,
...
CREATE OR REPLACE FUNCTION storage_inbound_max_active(
    _storage varchar
) RETURNS integer
AS $$
DECLARE
    _inbound_max_active int := NULL;
BEGIN
    SELECT
        inbound_max_active
    INTO
    ...
    IF _inbound_max_active IS NOT NULL THEN
