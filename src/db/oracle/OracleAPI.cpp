                             ...
                             const std::string & userDN, const std::string & voName, const std::string & credID)
{
    ...
    try {
        ...
        if (same_src_se) {
            soci::statement insertJob = (sql.prepare <<
                                         "INSERT INTO  t_job ( job_id, job_state, vo_name,submit_host, submit_time, user_dn, cred_id, source_se)"
            ...
            soci::statement insertJob = (sql.prepare <<
                                         "INSERT INTO  t_job ( job_id, job_state, vo_name,submit_host, submit_time, user_dn, cred_id)"
...
boost::optional<Job> OracleAPI::getJob(const std::string& jobId, bool archive)
{
    soci::session sql(*connectionPool);
        ...
        if (readyStmt.execute(true)) {
            do {
                ...
                if (diff > 200 && reuseJob != "Y") {
                    FTS3_COMMON_LOGGER_NEWLOG(WARNING) << "The transfer with file id " << fileId <<
                    " seems to be stalled, restart it" << commit;

                    sql <<
                    "UPDATE t_file SET start_time=NULL, file_state = 'SUBMITTED', reason='', transferhost='',finish_time=NULL, job_finished=NULL "
                    ...
                    if (reuseJob == "Y") {
                        ...
                        sql << " SELECT COUNT(*) FROM t_file WHERE job_id = :jobId ", soci::use(jobId), soci::into(
                            count);
                        if (count > 0)
                            terminateTime = count * 1000;

                        if (diff > terminateTime) {
                            FTS3_COMMON_LOGGER_NEWLOG(WARNING) << "The transfer with file id (reuse) " << fileId <<
                            " seems to be stalled, restart it" << commit;
...
std::map<std::string, int> OracleAPI::getFilesNumPerActivity(soci::session &sql, std::string src, std::string dst,
    std::string vo, int filesNum, std::set <std::string> &default_activities)
{
...
void OracleAPI::getQueuesWithPending(std::vector<QueueId>& queues)
{
    ...
    try {
        ...
        for (soci::rowset<soci::row>::const_iterator i = rs.begin(); i != rs.end(); ++i) {
            ...
            sql <<
            "select count(*) from t_optimize_active where source_se=:source_se and dest_se=:dest_se and datetime >= (sys_extract_utc(systimestamp) - interval '5' MINUTE)",
                soci::use(source_se),
                soci::use(dest_se),
    ...
    try {
        ...
        for (auto it = queues.begin(); it != queues.end(); ++it) {
            ...
            if (!manualConfigExists) {
                ...
                sql <<
                " select count(*) from t_file where source_se=:source_se and dest_se=:dest_se and file_state = 'ACTIVE' ",
...
void OracleAPI::getMultihopJobs(
...
{
    //auto-pick and manual replica logic must be added here
...
bool pairCompare(std::pair<std::pair<std::string, std::string>, int> i,
    std::pair<std::pair<std::string, std::string>, int> j)
{
...
int OracleAPI::getBestNextReplica(soci::session& sql, const std::string & job_id, const std::string & vo_name)
{
    ...
    try {
        ...
        if (!pair.empty()) {
            ...
            std::pair<std::pair<std::string, std::string>, int> minValue = *min_element(pair.begin(), pair.end(),
                pairCompare);
            ...
            sql <<
            "select file_id from t_file where file_state='NOT_USED' and source_se=:source_se and dest_se=:dest_se and job_id=:job_id",
...
void OracleAPI::listJobs(const std::vector<std::string>& inGivenStates,
...
void OracleAPI::listDmJobs(const std::vector<std::string>& inGivenStates,
        const std::string& forDN, const std::string& voName,
        const std::string& src, const std::string& dst,
        std::vector<JobStatus>& jobs)
{
...
void OracleAPI::getTransferStatuses(const std::string& requestID,
...
void OracleAPI::getDmStatuses(const std::string& requestID, bool archive,
...
boost::optional<StorageElement> OracleAPI::getStorageElement(const std::string& seName)
{
    soci::session sql(*connectionPool);
...
void OracleAPI::addStorageElement(const std::string& seName, const std::string& state)
{
    soci::session sql(*connectionPool);
...
void OracleAPI::updateStorageElement(const std::string& seName, const std::string& state)
{
    soci::session sql(*connectionPool);

    try {
        sql.begin();
        sql << "UPDATE t_se SET STATE = :state "
            "WHERE name = :name",
        ...
        int process_id, double filesize, double duration, bool retry)
{
    try {
        ...
        query <<
            "   , pid = :pid, filesize = :filesize, tx_duration = :duration, throughput = :throughput, current_failures = :current_failures "
...
bool OracleAPI::updateJobTransferStatusInternal(soci::session& sql, std::string job_id, const std::string status, int pid)
{
    ...
    try {
        ...
        if (job_id.empty())
            sql << " SELECT job_id from t_file where pid=:pid and job_finished is NOT NULL", soci::use(pid), soci::into(
                job_id);
...
void OracleAPI::updateFileTransferProgressVector(std::vector<fts3::events::MessageUpdater>& messages)
{
    soci::session sql(*connectionPool);
    ...
    try {
        ...
        soci::statement stmt = (sql.prepare <<
            "UPDATE t_file SET throughput = :throughput, transferred = :transferred WHERE file_id = :fileId  AND "
...
void OracleAPI::getCancelJob(std::vector<int>& requestIDs)
{
    ...
    try {
        soci::rowset<soci::row> rs = (sql.prepare <<
            " select distinct pid, file_id from t_file where "
        ...
        soci::statement stmt1 = (sql.prepare <<
            "UPDATE t_file SET  job_finished = sys_extract_utc(systimestamp) "
...
void OracleAPI::forkFailed(const std::string& jobId)
{
    soci::session sql(*connectionPool);
    ...
    try {
        ...
        sql << "UPDATE t_job "
               " SET job_state='FAILED', job_finished=sys_extract_utc(systimestamp), finish_time=sys_extract_utc(systimestamp),"
               "    reason='Transfer failed to fork, check fts3server.log for more details'"
               " WHERE job_id=:job_id",
            soci::use(jobId);
...
bool OracleAPI::markAsStalled(const std::vector<fts3::events::MessageUpdater>& messages, bool diskFull)
{
    soci::session sql(*connectionPool);
    ...
    if (diskFull) {
        ...
        transfer_message =
            "no FTS server had updated the transfer status the last 600 seconds, probably stalled in " + hostname;
    }
    ...
    try
    {
        for (auto iter = messages.begin(); iter != messages.end(); ++iter) {
            ...
            if (rs.begin() != rs.end()) {
                ok = true;
                updateFileTransferStatusInternal(sql, 0.0, (*iter).job_id(), (*iter).file_id(), transfer_status,
    ...
    try {
        ...
        if (count) {
            ...
            sql << "INSERT INTO t_bad_ses (se, message, addition_time, admin_dn, vo, status, wait_timeout) "
                ...
                soci::use(storage), soci::use(msg), soci::use(adminDn), soci::use(voName), soci::use(
                status), soci::use(timeout);
        }
    ...
    const std::vector<std::string> &groupMembers)
{
    ...
    try {
        ...
        std::string member;
        soci::statement stmt = (sql.prepare <<
            "INSERT INTO t_group_members(member, groupName) "
    ...
    try {
        ...
        if (sql.got_data()) {
            return std::unique_ptr < std::pair < std::string, std::string >>
                                                              (new std::pair<std::string, std::string>(source,
                                                                  destination));
        }
    }
    catch (std::exception &e) {
...
void OracleAPI::addShareConfig(const ShareConfig& cfg)
{
    ...
    try {
        ...
        sql <<
            "INSERT /*+ IGNORE_ROW_ON_DUPKEY_INDEX (t_link_config, T_LINK_CONFIG_PK) */ INTO t_share_config (source, destination, vo, active) "
...
int OracleAPI::getRetry(const std::string & jobId)
{
    ...
    try {
        ...
        if (nRetries > 0) {
            ...
            sql << "select count(*), count(distinct file_index) from t_file where job_id=:job_id", soci::use(
                jobId), soci::into(mreplicaCount), soci::into(mreplica);
...
int OracleAPI::getMaxTimeInQueue()
{
    ...
    try {
        ...
        sql <<
            "SELECT max_time_queue FROM "
            " (SELECT rownum as rn, max_time_queue FROM "
            "  t_server_config WHERE vo_name IS NULL OR vo_name = '*') WHERE rn = 1",
...
void OracleAPI::setGlobalLimits(const int* maxActivePerLink, const int* maxActivePerSe)
{
    ...
    try {
        sql << "SELECT max_per_link FROM t_server_config WHERE vo_name IS NULL or vo_name = '*'",
            soci::into(existsLink);
        sql << "SELECT max_per_se FROM t_server_config WHERE vo_name IS NULL or vo_name = '*'",
            soci::into(existsSe);
...
std::vector< std::pair<std::string, std::string> > OracleAPI::getAllPairCfgs()
{
    ...
    try {
        soci::rowset<soci::row> rs = (sql.prepare <<
            "select source, destination from t_link_config where source <> '*' and destination <> '*'");
...
void OracleAPI::updateProtocol(std::vector<fts3::events::Message>& messages)
{
    ...
    try {
        ...
        for (auto iter = messages.begin(); iter != messages.end(); ++iter) {
            ...
            fts3::events::Message msg = *iter;
            if (msg.transfer_status().compare("UPDATE") == 0) {
                fileId = msg.file_id();
                filesize = msg.filesize();
                internalParams << "nostreams:" << static_cast<int> (msg.nostreams()) << ",timeout:"
                    << static_cast<int> (msg.timeout()) << ",buffersize:" << static_cast<int> (msg.buffersize());
...
void OracleAPI::cancelJobsInTheQueue(const std::string& dn, std::vector<std::string>& jobs)
{
    ...
    try {
        // bear in mind that in this case we do not care about NOT_USED because we are operating at the level of a job
        soci::rowset<soci::row> rs = (
            sql.prepare <<
            " SELECT job_id "
                " FROM t_job "
...
void OracleAPI::transferLogFileVector(std::map<int, fts3::events::MessageLog>& messagesLog)
{
    soci::session sql(*connectionPool);
    ...
    try {
        soci::statement stmt = (sql.prepare <<
            " update t_file set t_log_file=:filePath, t_log_file_debug=:debugFile "
...
std::vector<TransferState> OracleAPI::getStateOfDeleteInternal(soci::session &sql, const std::string &jobId, int fileId)
{
    TransferState ret;
...
std::vector<TransferState> OracleAPI::getStateOfTransferInternal(soci::session &sql, const std::string &jobId,
    int fileId)
{
    TransferState ret;
    std::vector<TransferState> temp;
    ...
    try {
        ...
        for (it = rs.begin(); it != rs.end(); ++it) {
            ...
            ret.file_state = it->get<std::string>("FILE_STATE");
            if (ret.file_state == "SUBMITTED") {
                ret.timestamp = (soci::getTimeT(*it, "SUBMIT_TIME") * 1000);
            }
            else if (ret.file_state == "STAGING") {
                ret.timestamp = (soci::getTimeT(*it, "SUBMIT_TIME") * 1000);
            }
                ...
                ret.timestamp = (soci::getTimeT(*it, "START_TIME") * 1000);
            }
            else {
                ret.timestamp = (milliseconds_since_epoch());
            }
...
std::vector<TransferState> OracleAPI::getStateOfTransfer(const std::string &jobId, int fileId)
{
    soci::session sql(*connectionPool);
    std::vector<TransferState> temp;
...
void OracleAPI::checkSanityState()
{
    ...
    try {
        if(hashSegment.start == 0)
        {
            ...
            soci::statement stmtDel1 = (sql.prepare <<
                "SELECT COUNT(*) FROM t_dm where job_id=:jobId AND file_state in ('DELETE','STARTED') ",
                soci::use(job_id), soci::into(numberOfFilesDelete));

            soci::statement stmtDel2 = (sql.prepare <<
                "UPDATE t_dm SET "
                "    file_state = 'FAILED', job_finished = sys_extract_utc(systimestamp), finish_time = sys_extract_utc(systimestamp), "
                "    reason = 'Force failure due to file state inconsistency' "
            ...
            for (soci::rowset<std::string>::const_iterator i = rs.begin(); i != rs.end(); ++i) {
                try {
                    ...
                    if(numberOfFiles > 0 && (mreplica == "N" || mreplica == "Y" || mreplica == "H") &&  !(replicaJobCountAll > 1 && replicaJob == 1))
                    {
                        ...
                        // all files terminal state but job in ('ACTIVE','READY','SUBMITTED', 'STAGING')
                        if(numberOfFiles == terminalState)
                        {
                            if (allCanceled > 0) {
                                stmt2.execute(true);
                            }
                                //non canceled, check other states: "FINISHED" and FAILED"
                            else {
                                // all files finished
                                if (numberOfFiles == allFinished) {
                                    stmt3.execute(true);
                                }
                                else {
                                    // all files failed
                    ...
                    if(mreplica == "R" ||  (replicaJobCountAll > 1 && replicaJob == 1)) {
                        ...
                        soci::rowset<soci::row> rsReplica = (sql.prepare <<
                            ...
                            " from t_file where job_id=:job_id group by file_state order by null ",
                            soci::use(job_id)
                        );

                        sql << "SELECT job_state from t_job where job_id=:job_id",
                            soci::use(job_id), soci::into(job_state);
                        ...
                        for (iRep = rsReplica.begin(); iRep != rsReplica.end(); ++iRep) {
                            ...
                            if(file_state == "FINISHED") {
                                ...
                                if(job_state != "FINISHED") {
                                    //set the job_state to finished if at least one finished
                                    stmt3.execute(true);
                                }
                                break;
                            }
            ...
            soci::rowset<std::string> rs2 = (sql.prepare <<
                " select  j.job_id from t_job j inner join t_file f on (j.job_id = f.job_id) "
                " where j.job_finished is not NULL and f.file_state in ('SUBMITTED','ACTIVE') ");
            ...
            for (auto i444 = rs444.begin(); i444 != rs444.end(); ++i444) {
                ...
                if(mreplica == "R" ||  (replicaJobCountAll > 1 && replicaJob == 1)) {
                    ...
                    for (auto iRep = rsReplica.begin(); iRep != rsReplica.end(); ++iRep) {
                        std::string file_state = iRep->get<std::string>("FILE_STATE");
            ...
            soci::rowset<std::string> rs3 = (sql.prepare <<
                " select  j.job_id from t_job j inner join t_dm f on (j.job_id = f.job_id) "
                " where j.job_finished >= (sys_extract_utc(systimestamp) - interval '24' HOUR ) and "
            ...
            soci::rowset<std::string> rsCheckHosts = (sql.prepare <<
                " SELECT hostname "
                " FROM t_hosts "
            ...
            for (auto irsCheckHosts = rsCheckHosts.begin(); irsCheckHosts != rsCheckHosts.end(); ++irsCheckHosts) {
                ...
                soci::rowset<soci::row> rsCheckHostsActive = (sql.prepare <<
                    " SELECT file_id, job_id from t_file where file_state  = 'ACTIVE' and transferHost = :transferHost ", soci::use(deadHost)
                );
                for (auto iCheckHostsActive = rsCheckHostsActive.begin(); iCheckHostsActive != rsCheckHostsActive.end(); ++iCheckHostsActive) {
...
void OracleAPI::getFilesForNewSeCfg(std::string source, std::string destination, std::string vo, std::vector<int> &out)
{
    ...
    try {
        soci::rowset<long long> rs = (sql.prepare <<
            " select f.file_id "
            " from t_file f  "
...
void OracleAPI::getFilesForNewGrCfg(std::string source, std::string destination, std::string vo, std::vector<int> &out)
{
    ...
    try {
        ...
        if (source != "*")
            stmt.exchange(soci::use(source, "source"));
        if (destination != "*")
            stmt.exchange(soci::use(destination, "dest"));
...
void OracleAPI::getTransferRetries(int fileId, std::vector<FileRetry> &retries)
{
    ...
    try {
        soci::rowset<FileRetry> rs = (sql.prepare <<
            "SELECT * FROM t_file_retry_errors WHERE file_id = :fileId",
            soci::use(fileId));
...
void OracleAPI::updateHeartBeatInternal(soci::session &sql, unsigned *index, unsigned *count, unsigned *start,
    unsigned *end, std::string service_name)
{
    try {
        sql.begin();
...
void OracleAPI::snapshot(const std::string &vo_name, const std::string &source_se_p, const std::string &dest_se_p,
    const std::string &, std::stringstream &result)
{
    soci::session sql(*connectionPool);
    ...
    try {
        ...
        soci::statement st1((sql.prepare <<
            "select count(*) from t_file where "
        ...
        soci::statement st2((sql.prepare <<
            "select active from t_optimize_active where "
            " source_se=:source_se and dest_se=:dest_se",
            soci::use(source_se),
            soci::use(dest_se),
        ...
        soci::statement st41((sql.prepare <<
            "select avg(throughput) from t_file where  "
            " source_se=:source_se and dest_se=:dest_se "
            " AND  file_state in ('ACTIVE','FINISHED') "
            " AND (job_finished is NULL OR  job_finished >= (sys_extract_utc(systimestamp) - interval '60' minute)) "
            " AND throughput > 0 AND throughput is not NULL ",
            soci::use(source_se),
            soci::use(dest_se),
            soci::into(throughput1h, isNull2)));

        soci::statement st42((sql.prepare <<
            "select avg(throughput) from t_file where  "
            " source_se=:source_se and dest_se=:dest_se "
        ...
        soci::statement st43((sql.prepare <<
            "select avg(throughput) from t_file where  "
            " source_se=:source_se and dest_se=:dest_se "
        ...
        soci::statement st5((sql.prepare <<
            "select * from (select reason, count(reason) as c from t_file where "
        ...
        soci::statement st6((sql.prepare <<
            "select count(*) from t_file where "
        ...
        soci::statement st7((sql.prepare <<
            "select count(*) from t_file where "
...
bool OracleAPI::bandwidthChecker(soci::session &sql, const std::string &source_hostname,
    const std::string &destination_hostname, int &bandwidthIn)
{
...
std::string OracleAPI::getBandwidthLimitInternal(soci::session &sql, const std::string &source_hostname,
    const std::string &destination_hostname)
{
...
void OracleAPI::setBandwidthLimit(const std::string &source_hostname, const std::string &destination_hostname,
    int bandwidthLimit)
{
    soci::session sql(*connectionPool);
        ...
        if (!source_hostname.empty()) {
            ...
            if (countSource == 0 && bandwidthLimit > 0) {
                ...
                if (bandwidthLimit == -1) {
                    ...
                    sql.begin();
                    sql <<
                    "update t_optimize set throughput=:throughput where source_se=:source_se  and throughput is not NULL",
                        soci::use(bandwidthLimit), soci::use(source_hostname);
                    sql.commit();
                }
            }
        ...
        if (!destination_hostname.empty()) {
            ...
            if (countDest == 0 && bandwidthLimit > 0) {
                ...
                if (bandwidthLimit == -1) {
                    ...
                    sql.begin();
                    sql <<
                        "update t_optimize set throughput=:throughput where dest_se=:dest_se  and throughput is not NULL",
                        soci::use(bandwidthLimit), soci::use(destination_hostname);
                    sql.commit();
                }
...
bool OracleAPI::isProtocolUDT(const std::string &source_hostname, const std::string &destination_hostname)
{
    ...
    try {
        ...
        sql <<
            " select udt from t_optimize where (source_se = :source_se OR source_se = :dest_se)  and udt is not NULL ",
...
bool OracleAPI::isProtocolIPv6(const std::string &source_hostname, const std::string &destination_hostname)
{
    ...
    try {
        ...
        sql <<
            " select ipv6 from t_optimize where (source_se = :source_se OR source_se = :dest_se)  and ipv6 is not NULL ",
...
void OracleAPI::setGlobalTimeout(int timeout)
{
    ...
    try {
        sql << "SELECT global_timeout FROM t_server_config WHERE vo_name IS NULL or vo_name = '*'",
            soci::into(timeoutLocal, isNullTimeout);
        if (!sql.got_data()) {
            sql.begin();
...
int OracleAPI::getSecPerMb()
{
    ...
    try {
        ...
        sql << "SELECT sec_per_mb FROM t_server_config WHERE vo_name IS NULL OR vo_name = '*'", soci::into(seconds,
            isNullSeconds);
...
void OracleAPI::setSecPerMb(int seconds)
{
    ...
    try {
        sql << "SELECT sec_per_mb FROM t_server_config WHERE vo_name IS NULL OR vo_name = '*'",
            soci::into(secondsLocal, isNullSeconds);
...
void OracleAPI::setSourceMaxActive(const std::string &source_hostname, int maxActive)
{
    ...
    try {
        sql << "select source_se from t_optimize where source_se = :source_se and active is not NULL ",
            soci::use(source_hostname), soci::into(source_se, isNullSourceSe);
...
void OracleAPI::setDestMaxActive(const std::string &destination_hostname, int maxActive)
{
    ...
    try {
        sql << "select dest_se from t_optimize where dest_se = :dest_se and active is not NULL",
            soci::use(destination_hostname), soci::into(dest_se, isNullDestSe);
...
void OracleAPI::updateBringOnlineToken(std::map<std::string, std::map<std::string, std::vector<int> > > const &jobs,
...
void OracleAPI::getFilesForDeletion(std::vector<DeleteOperation>& delOps)
{
    ...
    try {
        ...
        if (!messages.empty()) {
            std::vector<fts3::events::MessageBringonline>::iterator iterUpdater;
        ...
        for (soci::rowset<soci::row>::const_iterator i2 = rs2.begin(); i2 != rs2.end(); ++i2) {
            ...
            for (soci::rowset<soci::row>::const_iterator i = rs.begin(); i != rs.end(); ++i) {
                ...
                for (soci::rowset<soci::row>::const_iterator i3 = rs3.begin(); i3 != rs3.end(); ++i3) {
                    ...
                    int file_id = static_cast<int>(row.get < long
                    long > ("FILE_ID"));
...
void OracleAPI::getStagingFilesForCanceling(std::set< std::pair<std::string, std::string> >& files)
{
    ...
    {
        soci::rowset<soci::row> rs = (sql.prepare <<
            " SELECT job_id, file_id, source_surl, bringonline_token from t_file WHERE "
            "  file_state='CANCELED' and job_finished is NULL "
...
void OracleAPI::setCloudStorage(std::string const &storage, std::string const &appKey, std::string const &appSecret,
