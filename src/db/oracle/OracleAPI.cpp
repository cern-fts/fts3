...
bool OracleAPI::isTrAllowed(const std::string & source_hostname, const std::string & destin_hostname) {
    ...
    std::string query_stmt1 = " select count(*) from  t_file, t_job where t_file.file_state in ('READY','ACTIVE') and t_job.job_id = t_file.job_id and t_job.source_se=:1 ";
	    
    std::string query_stmt2 = " select count(*) from  t_file, t_job where t_file.file_state in ('READY','ACTIVE') and t_job.job_id = t_file.job_id and t_job.dest_se=:1";
