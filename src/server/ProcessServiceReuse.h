...
class ProcessServiceReuse: public ProcessService
{
public:
...
protected:
    ...
    void startUrlCopy(std::string const & job_id, std::list<TransferFile> const & files)
    {
        ...
        std::map<int, std::string> fileIds = generateJobFile(
                representative.jobId, files);
        ...
        if (ready)
        {
            ...
            for (iterFileIds = fileIds.begin(); iterFileIds != fileIds.end();
                    ++iterFileIds)
            {
                struct message_updater msg2;
                if (std::string(job_id).length() <= 37)
                {
                    strncpy(msg2.job_id, std::string(job_id).c_str(),
                            sizeof(msg2.job_id));
    ...
    void executeUrlcopy()
    {
        try
        {
            ...
            try
            {
                DBSingleton::instance().getDBObjectInstance()->getQueuesWithSessionReusePending(
                        queues);
            }
            catch (std::exception& e)
            {
