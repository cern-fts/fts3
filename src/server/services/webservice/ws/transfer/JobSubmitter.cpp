...
static Uri checkValidUrl(const std::string &uri)
{
    ...
    bool ok =
        u0.host.length() != 0 && u0.protocol.length() != 0 && u0.path.length() != 0 && u0.protocol.compare("file") != 0;
    if (!ok) {
    ...
    copyPinLifeTime(-1)
{
    ...
    for (auto it = job->transferJobElements.begin(); it < job->transferJobElements.end(); it++) {
        ...
        if (!tupple.selectionStrategy.empty() && tupple.selectionStrategy != "orderly" &&
            tupple.selectionStrategy != "auto")
...
std::string JobSubmitter::submit()
{
    ...
    if (!params.isParamSet(JobParameterHandler::COPY_PIN_LIFETIME)) {
        ...
        if (params.get(JobParameterHandler::COPY_PIN_LIFETIME) != "-1" && !srm_source)
            throw UserError(
                "The 'ping-lifetime' operation can be used only with source SEs that are using SRM protocol!");
    }
    ...
    if (!params.isParamSet(JobParameterHandler::BRING_ONLINE)) {
        ...
        if (params.get(JobParameterHandler::BRING_ONLINE) != "-1" && !srm_source)
            throw UserError(
                "The 'bring-online' operation can be used only with source SEs that are using SRM protocol!");
    }
...
void JobSubmitter::checkProtocol(std::string file, bool source)
{
    ...
    bool not_ok =
        // check protocol
        !(tmp.find("mock://") == 0 || tmp.find("root://") == 0 || tmp.find("srm://") == 0 ||
          tmp.find("gsiftp://") == 0 ||
          tmp.find("https://") == 0 || tmp.find("lfc://") == 0 || tmp.find("davs://") == 0)
        &&
        // check if lfn if it is the source
        (!source ||
         !(file.find("/") == 0 && file.find(";") == std::string::npos && file.find(":") == std::string::npos));
