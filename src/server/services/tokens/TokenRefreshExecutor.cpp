...
void TokenRefreshExecutor::run([[maybe_unused]] boost::any & ctx)
{
    ...
    try {
        ...
        FTS3_COMMON_LOGGER_NEWLOG(INFO) << "Obtained refreshed access token: "
                                        ...
                                        << "expiration_time=" << refreshed_token.expirationTime
                                        << commit;

        tokenRefreshService.registerRefreshedAccessToken(refreshed_token);
...
RefreshedToken TokenRefreshExecutor::performTokenRefresh()
{
    ...
    req.setRequestBody(getRefreshData());
    ...
    return {
        token.tokenId,
        parseJson(exchange_result, "access_token"),
        parseJson(exchange_result, "refresh_token", false),
        token.refreshToken,
        db::getUTC(std::stoi(parseJson(exchange_result, "expires_in")))
    };
}
