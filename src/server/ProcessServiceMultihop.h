...
protected:
    ...
    {
        std::map<std::string,
                std::queue<std::pair<std::string, std::list<TransferFile> > > > voQueues;
        DBSingleton::instance().getDBObjectInstance()->getMultihopJobs(
                voQueues);
        ...
        while (!empty)
        {
            ...
            for (auto vo_it = voQueues.begin(); vo_it != voQueues.end(); ++vo_it)
            {
                ...
                if (!vo_jobs.empty())
                {
                    ...
                    vo_jobs.pop();
                    ProcessServiceReuse::startUrlCopy(job.first, job.second);
                }
            }
        }
    }
