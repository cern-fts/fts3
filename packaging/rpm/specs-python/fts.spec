...
## add filter setup
%{?filter_setup:
    %filter_provides_in %{_alt_python_sitearch}.*\.so$
    %filter_setup
}

Name: fts
Version: 3.2.0
Release: 3.py%{?_alt_python_version}
...
wget http://python.org/ftp/python/%{_alt_python_version}/Python-%{_alt_python_version}.tgz
tar xzf Python-%{_alt_python_version}.tgz
cd Python-%{_alt_python_version}
./configure --prefix=${WD}/altpython/
...
PYTHON_FLAGS="-DPYTHON_LIBRARIES=${WD}/altpython/lib/python%{_alt_python_version}/"
PYTHON_FLAGS="$PYTHON_FLAGS -DPYTHON_INCLUDE_PATH=${WD}/altpython/include/python%{_alt_python_version}/"
PYTHON_FLAGS="$PYTHON_FLAGS -DPYTHON_INCLUDE_DIR=${WD}/altpython/include/python%{_alt_python_version}/"
PYTHON_FLAGS="$PYTHON_FLAGS -DPYTHON_EXECUTABLE=${WD}/altpython/bin/python%{_alt_python_version}"

%if %{?fedora}%{!?fedora:0} >= 18 || %{?rhel}%{!?rhel:0} >= 7
    %cmake -DMAINBUILD=ON -D CMAKE_BUILD_TYPE=RelWithDebInfo -D CMAKE_INSTALL_PREFIX='' ${PYTHON_FLAGS} ..
%else
    %cmake28 -DMAINBUILD=ON -D CMAKE_BUILD_TYPE=RelWithDebInfo -D CMAKE_INSTALL_PREFIX='' ${PYTHON_FLAGS} ..
%endif
